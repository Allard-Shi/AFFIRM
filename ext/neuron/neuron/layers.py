...
class VecInt(Layer):
    ...
    def __init__(self, indexing='ij', method='ss', int_steps=7,  z_fix=False, **kwargs):
        """        
        Parameters:
    ...
    def build(self, input_shape):
        ...
        if input_shape[-1] != len(input_shape) - 2 and self.z_fix is not True:
    ...
    def call(self, inputs):
        ...
        if self.z_fix:            
            loc_shift_split = tf.split(loc_shift, loc_shift.shape[-1], axis=-1)
            loc_shift_lst = [loc_shift_split[0],loc_shift_split[1], tf.zeros(tf.shape(loc_shift_split[0]),dtype=tf.float32)]            
#            print('loc_shift shape:',loc_shift_split[0].get_shape())
            loc_shift = tf.concat(loc_shift_lst, -1)

        # map transform across batch
        out = tf.map_fn(self._single_int, loc_shift, dtype=tf.float32)
#        print('out shape: ',out.get_shape())
#        out._keras_shape = out.get_shape()
        return out
